...
Irrlicht.prototype = {
  ...
  handleConnect: function( req, socket, head ) {
    ...
    var self = this
    var proxy = null
    var dest = this.mitm ? self.socket :
      'tcp://' + req.headers.host
    ...
    var proxy = this.mitm ?
      ...
      new TCP.connect( target, onConnection )
  ...
  handleRequest: function( req, res ) {
    ...
    if( routes.length > 0 ) {
      ...
      debug( 'REPLAY' )
      this.replay( req, res, options )
    } else {
      debug( 'PROXY' )
      this.proxy( req, res, options )
    }
   ...
   */
  record: function( req, res, options ) {
   ...
   * @return {Undefined}
   */
  replay: function( req, res, options ) {
